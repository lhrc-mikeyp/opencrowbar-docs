...
#. **Given** ... some background thing has happened
#. **When** ... take some action - generally return a data set
#. **Then** ... get some result - always return true/false
#. Special Purpose Steps (optional)
#. **Skip** ... will disable the test (please provide a reason after the
   skip)
#. **While** ... environment ... will only run the test in the
   environments included after the While (opposite of Unless)
#. **Unless** environment ... will skip the test in the environments
